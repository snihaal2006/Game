...
const VideoBackground: React.FC<VideoBackgroundProps> = ({ videoUrl }) => {
    ...
    const isBunny = videoUrl.includes('player.mediadelivery.net');
    
    let youtubeId = '';
    if (isYouTube) {
        const match = videoUrl.match(/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))([^&?]+)/);
        youtubeId = match ? match[1] : 'binRgF2kBG0'; // Default to requested ID if parsing fails
    }
        ...
        <div className="absolute inset-0 w-full h-full overflow-hidden pointer-events-none">
            ...
            {isYouTube ? (
                <iframe
                    ...
                    src={`https://www.youtube.com/embed/${youtubeId}?autoplay=1&mute=1&controls=0&modestbranding=1&rel=0&showinfo=0&loop=1&playlist=${youtubeId}&playsinline=1`}
                ...
                <iframe
                    ...
                    src={`${videoUrl}?autoplay=true&loop=true&muted=true&preload=true&controls=false`}
    ...
    return (
        <div className="p-8 rounded-2xl backdrop-blur-sm bg-black/50 border border-white/10">
            <div className="mb-8 text-center">
                ...
                <div className="text-white/80 flex flex-col items-center space-y-1">
                    <span className="relative group cursor-default">
                        ...
                        <span className="relative inline-block animate-pulse">Your Mission is going to start</span>
                    </span>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                {loginError && (
                    <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-3 text-red-400 text-sm text-center animate-pulse font-mono tracking-widest">
                        {loginError}
                    </div>
                )}
                <FormInput
                ...
                >
                    {isSubmitting ? 'Logging in...' : 'Let the Game Begin!'}
                </button>
            </form>
            ...
            <div className="mt-8">
                ...
                <div className="mt-6 grid grid-cols-3 gap-3">
                    <div className="flex flex-col items-center justify-center py-4 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm transition-colors hover:bg-white/10 hover:border-purple-500/50 group">
                        <span className="text-xl font-mono font-bold text-purple-400 drop-shadow-[0_0_8px_rgba(168,85,247,0.6)] transition-colors group-hover:text-purple-300">5</span>
                        <span className="text-[10px] tracking-widest text-white/60 uppercase mt-1 transition-colors group-hover:text-white/80">Chapters</span>
                    </div>
                    <div className="flex flex-col items-center justify-center py-4 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm transition-colors hover:bg-white/10 hover:border-purple-500/50 group">
                        <span className="text-xl font-mono font-bold text-purple-400 drop-shadow-[0_0_8px_rgba(168,85,247,0.6)] transition-colors group-hover:text-purple-300">15</span>
                        <span className="text-[10px] tracking-widest text-white/60 uppercase mt-1 transition-colors group-hover:text-white/80">Puzzles</span>
                    </div>
                    <div className="flex flex-col items-center justify-center py-4 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm transition-colors hover:bg-white/10 hover:border-purple-500/50 group">
                        <span className="text-xl font-mono font-bold text-purple-400 drop-shadow-[0_0_8px_rgba(168,85,247,0.6)] transition-colors group-hover:text-purple-300">3H</span>
                        <span className="text-[10px] tracking-widest text-white/60 uppercase mt-1 transition-colors group-hover:text-white/80">Mission Time</span>
                    </div>
                </div>
            </div>
